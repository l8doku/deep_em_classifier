...
def create_model(num_classes, batch_size, train_data_shape, dropout_rate=0.3,
    ...
    if not use_transformer:
        ...
        transformer_block = transformer.TransformerBlock(
            ...
            use_masking=False)
        add_coordinate_embedding = position.TransformerCoordinateEmbedding(
            transformer_depth,
...
def evaluate_test(model, X, y=None,
    ...
    if res_Y is not None:
        ...
        for stat_i, stat_name in zip(list(range(1, 5)), [u'FIX', u'SACC', u'SP', u'NOISE']):
...
def run(args):
    ...
    for i, video_name in enumerate(all_video_names):
        ...
        for stat_name in [u'FIX', u'SACC', u'SP', u'NOISE']:
